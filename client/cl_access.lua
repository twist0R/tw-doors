...
RegisterNetEvent("k20-jobmanager:playerBecameJob")
AddEventHandler("k20-jobmanager:playerBecameJob", function(job, name, notify)
...
-- AddEventHandler('k20-jobs:jobChanged', function(currentJob, previousJob)
...
function hasSecuredAccess(pId, pType)
    ...
    local job = exports["k20-base"]:getModule("LocalPlayer"):getVar("job")
    ...
    if secured.access.item ~= nil then
        ...
        for i, v in pairs(secured.access.item) do
            if exports["k20-inventory"]:hasEnoughOfItem(i, 1, false) then
                return true
            end
        end
    end
