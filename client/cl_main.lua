...
RegisterNetEvent('k20-doors:initial-lock-state')
AddEventHandler('k20-doors:initial-lock-state', function(pDoors)
    doors = pDoors
...
RegisterNetEvent('k20-doors:change-lock-state')
AddEventHandler('k20-doors:change-lock-state', function(pDoorId, pDoorLockState, pDoorForceUnlock)
...
local function listenForKeypress()
    ...
    Citizen.CreateThread(function()
        ...
        if not hasAccess and currentDoorLockState then
            exports["k20-ui"]:showInteraction('Locked', 'error')
        end
        ...
        while listening do
            ...
            if currentDoorLockState ~= newLockState then
                if #(GetOffsetFromEntityGivenWorldCoords(PlayerPedId(), currentDoorCoords)) <= 1.2 then
                    ...
                    if hasAccess then
                        exports["k20-ui"]:showInteraction(("[E] %s"):format(newLockState and 'Locked' or 'Unlocked'), newLockState and 'error' or 'success')
                    end
                else
                    idle = 100
                end
            ...
            if currentDoorId ~= nil and hasAccess and IsControlJustReleased(0, 38) and #(GetOffsetFromEntityGivenWorldCoords(PlayerPedId(), currentDoorCoords)) <= 1.2 then
                ...
                TriggerServerEvent("k20-doors:change-lock-state", currentDoorId, not currentDoorLockState)
            end

            Wait(idle)
        end

        exports["k20-ui"]:hideInteraction((not hasAccess or newLockState) and 'error' or 'success')
    end)
end
...
AddEventHandler("k20-doors:doorKeyFob", function()
    ...
    if currentZone ~= nil and bollards[currentZone].inside then
        ...
        local entity = exports['k20-target']:GetEntityPlayerIsLookingAt(10.0, 2.0, 16)
    ...
    TriggerServerEvent("k20-doors:change-lock-state", doorId, isBollard and (not isLocked and 6 or 0) or (not isLocked))
end)

Citizen.CreateThread(function()
    exports["k20-polyzone"]:AddBoxZone("mrpd_bollards_01", vector3(411.66, -1027.95, 29.24), 7.8, 23.4, {
        heading=0,
        minZ=28.14,
        maxZ=32.14
    })
    exports["k20-polyzone"]:AddBoxZone("mrpd_bollards_02", vector3(411.66, -1020.09, 29.34), 7.8, 23.4, {
        heading=0,
        minZ=28.14,
        maxZ=32.14
    })
    doors = DOOR_CONFIG
    TriggerServerEvent("k20-doors:request-lock-state")
end)

RegisterNetEvent("k20-doors:add")
AddEventHandler("k20-doors:add", function(pArgs, pEntity, pContext)
    if GetConvar("sv_environment", "prod") == "debug" then
        TriggerServerEvent("k20-door:add", GetEntityCoords(pEntity), GetEntityModel(pEntity))
    end
end)

AddEventHandler("k20-polyzone:enter", function(zone, data)
...
AddEventHandler("k20-polyzone:exit", function(zone)
    if zone == "mrpd_bollards_01" or zone == "mrpd_bollards_02" then
